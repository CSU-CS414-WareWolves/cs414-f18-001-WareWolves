...
class KingTest {
  ...
  void testMove(String point, boolean color, String move, boolean expected) {
    ...
    if (expected) {
      Assertions.assertEquals(testKing.boardLocation, new Point(move), "King should be in the new position");
    } else {
      Assertions.assertEquals(testKing.boardLocation, new Point(point), "King should not have moved");
    }
  }
}